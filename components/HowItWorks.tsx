...
const HowItWorks: React.FC = () => {
    return (
        <section id="how-it-works" className="py-20 sm:py-28 bg-slate-50 dark:bg-slate-900/70 backdrop-blur-sm">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-16">
                     <h2 className="text-3xl sm:text-4xl font-bold text-slate-900 dark:text-slate-100">Our Automated Workflow</h2>
                     <p className="mt-4 text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">A streamlined, four-step process from concept to deployment, powered entirely by AI.</p>
                </div>
                        ...
                        {HOW_IT_WORKS_STEPS.map((step, index) => (
                            <div key={index} className="relative flex items-center md:justify-center">
                                <div className={`relative flex items-start gap-6 w-full md:w-1/2 p-4 ${index % 2 === 0 ? 'md:pr-12' : 'md:pl-12 md:ml-auto md:text-right md:flex-row-reverse'}`}>
                                    ...
                                    <div className="flex-shrink-0 flex items-center justify-center h-14 w-14 rounded-full bg-white dark:bg-slate-800 border-2 border-sky-500 text-sky-500 dark:text-sky-400 text-2xl font-bold z-10">
                                        {index + 1}
                                    </div>
